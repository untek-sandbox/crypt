((exports, WebSocket, window) => {
  ...
  class NCALayerClient {
    ...
    /**
     * **Не рекомендуется использовать, разработчики NCALayer пометили как DEPRECATED (https://forum.pki.gov.kz/t/podpis-s-metkoj-vremeni-na-js/704/7)!**
     *
     * Подписать блок данных и сформировать CMS (CAdES) подпись с интегрированной меткой времени TSP.
     *
     * @param {String} storageType тип хранилища который следует использовать для подписания.
     *
     * @param {String | ArrayBuffer} data данные которые нужно подписать в виде строки Base64 либо
    ...
    static get fileStorageType() {
      return 'PKCS12'; //or AKKaztokenStore, AKKZIDCardStore, AKEToken72KStore, AKJaCartaStore
    }
...
async function connAndSign() {
  var ncaLayerClient = new NCALayerClient()

  try{
    await ncaLayerClient.connect();
  } catch (e) {
    alert(`Не удалось подключиться к NCALayer: ${e.toString()}`)
    return
  }

  let activeToken;
  try{
    activeToken = await ncaLayerClient.getActiveTokens();
  } catch (e) {
    alert(e.toString())
    return
  }

  var storageType = activeToken[0] || NCALayerClient.fileStorageType

  var base64EncodeSignature;

  try{
    base64EncodeSignature = await ncaLayerClient.createCAdESFromBase64(storageType, 'MTEK');
  } catch (e) {
    alert(e.toString())
    return
  }
  console.log(atob(base64EncodeSignature))
